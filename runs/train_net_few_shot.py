...
def train_epoch(
    ...
    for cur_iter, task_dict in enumerate(train_loader):
        ...
        if (cur_iter + 1) % cfg.TRAIN.VAL_FRE_ITER == 0:   # 
            ...
            # cu.save_checkpoint(cfg.OUTPUT_DIR, model, model_ema, optimizer, 999, cfg, model_bucket)  # save latest ckpt, overwrite
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    model.eval()
    val_meter.iter_tic()
    if cfg.TRAIN.DATASET == 'FSARVideoDataset':
        val_loader.dataset.split = 'test'
    ...
    val_meter.reset()
    if cfg.TRAIN.DATASET == 'FSARVideoDataset':
        val_loader.dataset.split = 'train'
